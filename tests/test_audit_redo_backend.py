...
def test_compute_envelope_exact_goal_picks_closest_not_max():
    from ui.backend.services.checkpoint_reader import compute_envelope
...
def test_pdk_fastpath_includes_tech_prefixed_models_layout():
    from ui.backend.services.env_probe import (
        _PDK_LIB_SUBPATHS,
        _PDK_MODEL_LIB,
        _PDK_TECH,
    )
...
def test_pdk_xschem_dir_resolves_with_pdk_root_only(tmp_path, monkeypatch):
    ...
    import ui.backend.routes.xschem as X
    from ui.backend.services.env_probe import _PDK_TECH
