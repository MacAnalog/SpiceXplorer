...
def test_autosave_default_is_cwd_relative(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    proj = _load_with(tmp_path, ws_root=".")
    opt = _NoopOpt(proj)  # no output_root → ./auto_save (unchanged default)
    assert (tmp_path / "auto_save").resolve() in opt.autosave_checkpoint_dir.resolve().parents
...
def test_work_root_env_override(tmp_path, monkeypatch):
    ...
    import importlib
    from ui.backend import app_config
