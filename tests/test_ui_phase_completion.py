...
def test_parse_meas_candidates_extracts_result_names():
    from ui.backend.services.netlist_parser import parse_meas_candidates
...
def test_count_iters_json_and_csv(tmp_path):
    import json
    from ui.backend.routes.checkpoint import _count_iters
...
# ---------- §16 #7: score_service penalty dedup ----------

def test_normalized_penalties_zero_when_met_positive_when_violated():
    from ui.backend.services.score_service import _normalized_penalties

    assert _normalized_penalties(0.0, 10.0) == (0.0, 0.0)
    assert _normalized_penalties(-5.0, 10.0) == (0.0, 0.0)
    lin, sig = _normalized_penalties(5.0, 10.0)
    assert lin > 0.0 and sig > 0.0
