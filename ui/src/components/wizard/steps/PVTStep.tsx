      ...
      <div className="space-y-4 p-4">
        ...
        {corners.length > 0 && (
          <Field label="Active corner" hint="Which corner the optimizer runs against (Phase 1).">
            <select
              ...
              className="w-full min-w-0 rounded-md border border-zinc-300 bg-white px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500"
            >
              {corners.map((c) => (
        ...
        {corners.map((c, i) => (
          <div key={i} className="space-y-2 rounded-md border border-zinc-200 p-3">
            ...
            <div className="space-y-1.5 rounded border border-zinc-100 bg-zinc-50 p-2">
              <div className="flex items-center justify-between">
