              ...
              <Panel>
                ...
                <PanelBody className="space-y-1.5">
                  {result.testbenches.map((tb) => (
                    <div
                      key={tb.name}
                      className="rounded border border-border bg-hairline/40"
                    >
                      ...
                      {tb.log_tail && (
                        <details className="border-t border-border px-2 py-1 text-[11px]">
                          ...
                          <pre className="mt-1 max-h-60 overflow-auto rounded bg-[#1c1c1f] p-2 font-mono text-[10px] leading-tight text-zinc-100">
                            {tb.log_tail}
                          </pre>
                        </details>
                      )}
            ...
            {result.trial && (
              <Panel>
                ...
                <PanelBody className="space-y-2">
                  {result.trial.error ? (
                    <p className="rounded bg-danger-soft px-2 py-1 font-mono text-[11px] text-danger">
                      {result.trial.error}
                    </p>
                  ) : (
                    <>
                      ...
                      {Object.keys(result.trial.log_tails || {}).length > 0 && (
                        <div className="space-y-1 pt-1">
                          ...
                          {Object.entries(result.trial.log_tails).map(([name, tail]) => (
                            <details
                              key={name}
                              className="rounded border border-border bg-hairline/40 px-2 py-1 text-[11px]"
                            >
                              <summary className="cursor-pointer text-muted hover:text-fg">
                                <FileText className="mr-1 inline h-3 w-3" />
