    ...
    <div className="relative h-full w-full">
      ...
      <div className="pointer-events-none absolute right-3 top-3 flex flex-col gap-1 text-xs">
        <button
          type="button"
          onClick={resetView}
          className="pointer-events-auto rounded border border-border bg-panel/90 px-2 py-1 text-fg hover:bg-panel"
        >
          Fit
        </button>
      </div>
      ...
      <div className="flex-1 overflow-auto p-3">
        ...
        {multiLine.map(([k, v]) => (
          <div key={k} className="mt-3">
            ...
            <pre className="max-h-48 overflow-auto rounded border border-border bg-bg/60 p-2 font-mono text-[11px] leading-snug">
              {v}
            </pre>
          </div>
        ))}
      ...
      {navigable && (
        <div className="border-t border-border bg-bg/40 px-3 py-2">
          <button
            type="button"
            onClick={onDescend}
            className="w-full rounded border border-primary/40 bg-primary/15 px-2 py-1 text-primary hover:bg-primary/25"
          >
