v {xschem version=3.4.5 file_version=1.2
}
G {}
K {}
V {}
S {}
E {}
T {1.45 < 1.5(nom) < 1.55V} 370 -890 0 0 0.4 0.4 {}
T {4uA} 880 -150 0 0 0.4 0.4 {}
T {0.7 < 0.8(nom) < 0.9V} 480 -620 0 0 0.4 0.4 {}
T {~0.7V} 760 -300 2 1 0.3 0.3 {}
T {~0.38V} 860 -530 2 1 0.3 0.3 {}
T {~0.78V} 860 -640 2 1 0.3 0.3 {}
N 900 -710 960 -710 {
lab=gate_p}
N 820 -660 820 -620 {
lab=gate_p}
N 820 -660 900 -660 {
lab=gate_p}
N 820 -680 820 -660 {
lab=gate_p}
N 900 -710 900 -660 {
lab=gate_p}
N 880 -710 900 -710 {
lab=gate_p}
N 760 -280 860 -280 {
lab=gate_n}
N 760 -280 760 -240 {
lab=gate_n}
N 740 -280 760 -280 {
lab=gate_n}
N 640 -250 640 -160 {
lab=vss}
N 900 -250 900 -160 {
lab=vss}
N 760 -180 760 -160 {
lab=vss}
N 640 -160 760 -160 {
lab=vss}
N 760 -210 820 -210 {
lab=vss}
N 760 -160 900 -160 {
lab=vss}
N 900 -280 960 -280 {
lab=vss}
N 580 -280 640 -280 {
lab=vss}
N 820 -860 820 -740 {
lab=vdd}
N 880 -860 880 -820 {
lab=vdd}
N 820 -860 880 -860 {
lab=vdd}
N 1000 -860 1000 -740 {
lab=vdd}
N 880 -790 940 -790 {
lab=vdd}
N 880 -860 1000 -860 {
lab=vdd}
N 1000 -710 1060 -710 {
lab=vdd}
N 760 -710 820 -710 {
lab=vdd}
N 1000 -640 1000 -620 {
lab=vout}
N 820 -560 820 -520 {
lab=tail}
N 900 -520 1000 -520 {
lab=tail}
N 1000 -560 1000 -520 {
lab=tail}
N 820 -590 1000 -590 {
lab=vss}
N 820 -520 900 -520 {
lab=tail}
N 420 -180 420 -160 {
lab=vss}
N 360 -310 380 -310 {
lab=ena_n}
N 360 -210 380 -210 {
lab=ena_n}
N 680 -280 740 -280 {
lab=gate_n}
N 1080 -590 1080 -500 {
lab=vinn}
N 740 -360 740 -280 {
lab=gate_n}
N 900 -520 900 -310 {
lab=tail}
N 640 -440 640 -310 {
lab=ibias_20u}
N 420 -210 480 -210 {
lab=vss}
N 480 -210 480 -160 {
lab=vss}
N 740 -440 740 -420 {
lab=ibias_20u}
N 740 -390 820 -390 {
lab=vss}
N 880 -760 880 -710 {
lab=gate_p}
N 860 -710 880 -710 {
lab=gate_p}
N 420 -160 480 -160 {lab=vss}
N 420 -360 420 -340 {lab=vdd}
N 420 -360 480 -360 {lab=vdd}
N 480 -360 480 -310 {lab=vdd}
N 420 -310 480 -310 {lab=vdd}
N 420 -860 420 -360 {lab=vdd}
N 420 -860 820 -860 {lab=vdd}
N 640 -440 740 -440 {lab=ibias_20u}
N 160 -440 640 -440 {lab=ibias_20u}
N 160 -590 780 -590 {lab=vinp}
N 260 -180 260 -160 {
lab=vss}
N 200 -310 220 -310 {
lab=d_ena}
N 200 -210 220 -210 {
lab=d_ena}
N 200 -260 200 -210 {
lab=d_ena}
N 260 -210 320 -210 {
lab=vss}
N 320 -210 320 -160 {
lab=vss}
N 260 -160 320 -160 {
lab=vss}
N 260 -260 260 -240 {lab=ena_n}
N 260 -360 260 -340 {lab=vdd}
N 260 -360 320 -360 {lab=vdd}
N 320 -360 320 -310 {lab=vdd}
N 260 -310 320 -310 {lab=vdd}
N 160 -260 200 -260 {lab=d_ena}
N 200 -310 200 -260 {
lab=d_ena}
N 160 -160 260 -160 {lab=vss}
N 260 -860 260 -360 {lab=vdd}
N 160 -860 260 -860 {lab=vdd}
N 260 -280 260 -260 {lab=ena_n}
N 360 -310 360 -260 {
lab=ena_n}
N 360 -260 360 -210 {
lab=ena_n}
N 420 -250 420 -240 {lab=ena}
N 480 -160 640 -160 {lab=vss}
N 320 -160 420 -160 {lab=vss}
N 260 -860 420 -860 {lab=vdd}
N 260 -260 360 -260 {lab=ena_n}
N 1040 -590 1080 -590 {
lab=vinn}
N 160 -500 1080 -500 {lab=vinn}
N 1000 -640 1140 -640 {lab=vout}
N 1000 -680 1000 -640 {
lab=vout}
N 670 -390 700 -390 {lab=ena}
N 830 -790 840 -790 {lab=ena}
N 690 -210 720 -210 {lab=ena_n}
N 420 -250 510 -250 {lab=ena}
N 420 -280 420 -250 {lab=ena}
C {devices/title.sym} 160 -30 0 0 {name=l5 author="(c) 2024-2025 H. Pretl, Apache-2.0 license"}
C {sg13g2_pr/sg13_lv_nmos.sym} 880 -280 0 0 {name=M5
l=x_dut_nfet_mirror_l
w=x_dut_nfet_mirror_w
ng=1
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_pmos.sym} 980 -710 0 0 {name=M4
l=x_dut_pfet_load_l
w=x_dut_pfet_load_w
ng=1
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_nmos.sym} 800 -590 0 0 {name=M1
l=x_dut_nfet_input_l
w=x_dut_nfet_input_w
ng=1
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_nmos.sym} 1020 -590 0 1 {name=M2
l=x_dut_nfet_input_l
w=x_dut_nfet_input_w
ng=1
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_pmos.sym} 840 -710 0 1 {name=M3
l=x_dut_pfet_load_l
w=x_dut_pfet_load_w
ng=1
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_nmos.sym} 660 -280 0 1 {name=M6
l=x_dut_nfet_mirror_ref_w
w=x_dut_nfet_mirror_ref_l
ng=1
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_nmos.sym} 740 -210 0 0 {name=M9
l=0.13u
w=0.5u
ng=1
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_pmos.sym} 860 -790 0 0 {name=M11
l=0.13u
w=1.5u
ng=1
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {devices/ipin.sym} 160 -440 0 0 {name=p1 lab=ibias_20u}
C {devices/iopin.sym} 160 -860 0 1 {name=p2 lab=vdd}
C {devices/iopin.sym} 160 -160 0 1 {name=p3 lab=vss}
C {devices/ipin.sym} 160 -590 0 0 {name=p4 lab=vinp}
C {devices/ipin.sym} 160 -500 0 0 {name=p5 lab=vinn}
C {devices/opin.sym} 1140 -640 0 0 {name=p6 lab=vout}
C {sg13g2_pr/sg13_lv_nmos.sym} 400 -210 0 0 {name=M7
l=0.13u
w=0.5u
ng=1
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_pmos.sym} 400 -310 0 0 {name=M8
l=0.13u
w=1.5u
ng=1
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {devices/ipin.sym} 160 -260 0 0 {name=p7 lab=d_ena}
C {sg13g2_pr/sg13_lv_nmos.sym} 720 -390 0 0 {name=M10
l=0.13u
w=0.5u
ng=1
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {devices/lab_wire.sym} 690 -210 0 0 {name=p8 sig_type=std_logic lab=ena_n}
C {devices/lab_wire.sym} 800 -280 0 0 {name=p9 sig_type=std_logic lab=gate_n}
C {devices/lab_wire.sym} 940 -710 0 0 {name=p10 sig_type=std_logic lab=gate_p}
C {devices/lab_wire.sym} 900 -440 0 0 {name=p11 sig_type=std_logic lab=tail}
C {sg13g2_pr/sg13_lv_nmos.sym} 240 -210 0 0 {name=M12
l=0.13u
w=0.5u
ng=1
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_pmos.sym} 240 -310 0 0 {name=M13
l=0.13u
w=1.5u
ng=1
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {devices/lab_wire.sym} 670 -390 0 0 {name=p12 sig_type=std_logic lab=ena}
C {sg13g2_pr/annotate_fet_params.sym} 1280 -310 0 0 {name=annot1 ref=M5}
C {sg13g2_pr/annotate_fet_params.sym} 1280 -520 0 0 {name=annot2 ref=M1}
C {sg13g2_pr/annotate_fet_params.sym} 1440 -520 0 0 {name=annot3 ref=M2}
C {sg13g2_pr/annotate_fet_params.sym} 1280 -720 0 0 {name=annot4 ref=M3}
C {sg13g2_pr/annotate_fet_params.sym} 1440 -720 0 0 {name=annot5 ref=M4}
C {devices/lab_wire.sym} 200 -160 0 0 {name=p13 sig_type=std_logic lab=vss}
C {devices/lab_wire.sym} 960 -280 1 0 {name=p14 sig_type=std_logic lab=vss}
C {devices/lab_wire.sym} 910 -590 1 0 {name=p15 sig_type=std_logic lab=vss}
C {devices/lab_wire.sym} 820 -390 1 0 {name=p16 sig_type=std_logic lab=vss}
C {devices/lab_wire.sym} 820 -210 1 0 {name=p17 sig_type=std_logic lab=vss}
C {devices/lab_wire.sym} 580 -280 3 1 {name=p18 sig_type=std_logic lab=vss}
C {devices/lab_wire.sym} 220 -860 1 0 {name=p19 sig_type=std_logic lab=vdd}
C {devices/lab_wire.sym} 1060 -710 1 0 {name=p20 sig_type=std_logic lab=vdd}
C {devices/lab_wire.sym} 940 -790 1 0 {name=p21 sig_type=std_logic lab=vdd}
C {devices/lab_wire.sym} 760 -710 3 1 {name=p22 sig_type=std_logic lab=vdd}
C {devices/lab_wire.sym} 510 -250 0 1 {name=p23 sig_type=std_logic lab=ena}
C {devices/lab_wire.sym} 830 -790 0 0 {name=p24 sig_type=std_logic lab=ena}
C {devices/lab_wire.sym} 320 -260 0 0 {name=p25 sig_type=std_logic lab=ena_n}
